...
3. **Local models.** In a separate terminal run `make ollama`. It starts Ollama with a 16K context and one model loaded at a time, which suits a 16 GB Mac. Then download the models (about 18 GB):
   ```sh
   for m in qwen3.5:9b qwen3.5:4b gemma4:12b qwen3-embedding:0.6b; do ollama pull "$m"; done
   ```
4. **Dependencies.** `cd backend && uv sync`, then `cd frontend && npm ci`.
...
- **npm:** npm has no equivalent setting, so pass a cutoff date when adding packages, for example `npm install <pkg> --before=$(date -v-7d +%F)`. Commit `package-lock.json`.
